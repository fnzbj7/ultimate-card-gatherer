...
export class JsonBaseController {
    ...
    @Get('/:id/full-name')
    async getJsonFullName(@Param('id') id: string): Promise<{fullName: string}> {
        return await this.jsonBaseRepository.getJsonFullName(+id);
    }

    @Post('/:id/upload-svg')
    @UseInterceptors(FileInterceptor('iconSvg'))
    async uploadSvg(@Param('id') id: string,@UploadedFile() file: Express.Multer.File) {
    ...
    @Get(':id/icon-svg')
        async getIconSvg(@Param('id') id: string, @Res() res: Response): Promise<void> {
        const jsonBase: JsonBase = await this.jsonBaseRepository.getSingleJsonBase(+id);
        res.setHeader('Content-Type', 'image/svg+xml');
        if(jsonBase.icon) {
            res.send(jsonBase.icon);
        } else {
            res.send(`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="23" height="32" viewBox="0 0 23 32">
            <title>dkm</title>
